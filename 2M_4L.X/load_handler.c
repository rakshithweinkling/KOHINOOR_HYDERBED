...
void setLightState(uint8_t touchID, bool state) {
    switch (touchID) {
        ...
        case T2: if (state) L2_SetHigh(); else L2_SetLow(); break;
        case T3: if (state) L3_SetHigh(); else L3_SetLow(); break;
        case T4: if (state) L4_SetHigh(); else L4_SetLow(); break;
...
void handleFanToggle(FanControl *fan) {
    ...
    if (*fan->state == 1) {
        ...
        *fan->led = *fan->fan;
           ...
           EUSART_Write(fan->toggleChar);
           EUSART_Write(fan->baseChar + (*fan->fan - 1));
